...
with st.sidebar:
    ...
    # NEW: toggle to let watchlist override filters completely
    ignore_filters_when_watchlist = st.checkbox(
        "Ignore filters when watchlist is populated",
        value=True,
        help="When checked and a watchlist is provided, all watchlist symbols are shown regardless of filters."
    )
    ...
    # UI CHANGE YOU REQUESTED: allow min volume = 0
...
def load_symbols():
    """Load US symbols (NASDAQ + otherlisted), robust to missing ETF column."""
    nasdaq = pd.read_csv(
    ...
    # Ensure consistent columns even if ETF is missing on one side
    nas_sub = nasdaq[["Symbol", "Exchange"]].copy()
    oth_sub = other[["Symbol", "Exchange"]].copy()

    # Add ETF column if present, else fill with NaN
    if "ETF" in nasdaq.columns:
        nas_sub["ETF"] = nasdaq["ETF"]
    else:
        nas_sub["ETF"] = pd.NA

    if "ETF" in other.columns:
        oth_sub["ETF"] = other["ETF"]
    else:
        oth_sub["ETF"] = pd.NA

    df = pd.concat([nas_sub, oth_sub], ignore_index=True).dropna(subset=["Symbol"])
...
def scan_one(
    ...
    ignore_filters_when_watchlist: bool,
):
    try:
        ticker = sym["Symbol"]
        ...
        vol_last_daily = float(volume.iloc[-1])

        # Base price/volume filters – skipped if watchlist override is active
        if not (watchlist_active and ignore_filters_when_watchlist):
            if price > max_price or vol_last_daily < min_volume:
                return None
        ...
        avg10 = float(volume.mean()) if len(volume) > 0 else 0.0

        # Intraday 2m history for PM, VWAP, order flow, and *current* volume
        premarket_pct = None
        vwap_dist = None
        order_flow_bias = None
        vol_today = vol_last_daily  # fallback
        ...
        if intra is not None and not intra.empty and len(intra) >= 3:
            ...
            # Current intraday total volume (more "live" than daily bar)
            vol_today = float(ivol.sum())
        ...
        # RVOL using *current* intraday volume when available
        rvol10 = None
        if avg10 > 0 and vol_today is not None:
            rvol10 = vol_today / avg10
        ...
        return {
            ...
            "Exchange": exchange,
            "Price": round(price, 2),
            "Volume": int(vol_today),  # 🔥 live(ish) volume based on intraday sum
            "Score": score,
            "Prob_Rise%": prob_rise,
...
if df.empty:
    ...
    # Apply filters UNLESS watchlist override is active
    if not (watchlist_active and ignore_filters_when_watchlist):
