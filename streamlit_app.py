...
with st.sidebar:
    ...
    # V10: allow watchlist to ignore filters
    ignore_filters_for_watchlist = st.checkbox(
        "V10: Ignore filters when watchlist is populated",
        value=True,
        help="When checked, symbols from your watchlist bypass price/volume & other filters."
    )
    ...
    # V10: extra output toggle (doesn't remove anything existing)
    st.markdown("---")
    st.subheader("V10 Output Options")
    show_v10_extras = st.checkbox(
        "Show V10 extra stats / commentary block",
        value=True,
        help="Extra derived stats & commentary under each symbol."
    )
...
# ========================= EXTRA V10 STATS (non-UI) =========================
def compute_v10_stats(close_series: pd.Series, volume_series: pd.Series):
    """
    Compute a few extra lightweight stats for V10 without changing core behavior.
    Returns (atr10, slope_10d).
    """
    atr10 = None
    slope_10d = None
    ...
    try:
        if len(close_series) >= 5:
            # Approximate ATR(10) using absolute pct change
            log_returns = close_series.pct_change().dropna()
            if not log_returns.empty:
                atr10 = float((log_returns.abs().rolling(10).mean().iloc[-1]) * 100)

        if len(close_series) >= 3:
            # Simple slope over last N points
    ...
    return atr10, slope_10d


# ========================= CORE SCAN =========================
def scan_one(sym, enable_enrichment: bool, enable_ofb_filter: bool, min_ofb: float, ignore_filters_for_watchlist: bool):
    try:
        ticker = sym["Symbol"]
        ...
        # Respect watchlist override for price/volume filters
        if not (ignore_filters_for_watchlist and is_watchlist_symbol):
        ...
        # Extra V10 stats
        atr10, slope_10d = compute_v10_stats(close, volume)
        ...
        return {
            ...
            "Exchange": exchange,
            "Price": round(price, 2),
            "Volume": int(vol_last),             # volume signal
            "Score": score,
            "Prob_Rise%": prob_rise,
            ...
            "ATR10_est": round(atr10, 2) if atr10 is not None else None,
            "Slope_10D": round(slope_10d, 4) if slope_10d is not None else None,
        }
...
def run_scan(
    ...
    universe_mode: str,
    volume_rank_pool: int,
    ignore_filters_for_watchlist: bool,
):
...
# ========================= MAIN DISPLAY =========================
with st.spinner("Scanning (10-day momentum, V9/V10 hybrid universe)…"):
    df = run_scan(
        watchlist_text,
        max_universe,
        enable_enrichment,
...
if df.empty:
    ...
    has_watchlist = bool(watchlist_text.strip())

    # Apply filters (unless we're in watchlist+override mode)
    if not (has_watchlist and ignore_filters_for_watchlist):
    ...
    for _, row in df.iterrows():
        ...
        # Column 3: Microstructure + AI commentary + V10 extras
        ...
        # AI commentary line (existing behavior)
        c3.markdown(f"🧠 **AI View:** {row.get('AI_Commentary', '—')}")

        # V10 extra stats block (optional)
        if show_v10_extras:
            atr_val = row.get("ATR10_est", None)
            slope_val = row.get("Slope_10D", None)
            extra_bits = []
            if atr_val is not None:
                extra_bits.append(f"ATR10≈{atr_val}%")
            if slope_val is not None:
                extra_bits.append(f"Slope10D={slope_val}")
            if extra_bits:
                c3.markdown("📊 **V10 Extras:** " + " | ".join(extra_bits))
    ...
    csv_cols = [
        ...
        "AI_Commentary", "ATR10_est", "Slope_10D",
    ]
    ...
    st.download_button(
        ...
        file_name="v10_10day_momentum_screener_hybrid.csv",
        mime="text/csv",
    )
