...
DEFAULT_MIN_VOLUME    = 100_000      # default value (UI can still go down to 0)
...
with st.sidebar:
    ...
    # 🔧 allow min volume down to 0 (only change to this control)
...
def load_symbols():
    """Load US symbols (NASDAQ + otherlisted) safely without KeyErrors."""
    nasdaq = pd.read_csv(
    ...
    # Normalize columns to avoid KeyError on ETF / Exchange / Symbol
    for df in (nasdaq, other):
        if "Symbol" not in df.columns:
            # otherlisted usually has ACT Symbol
    ...
    nasdaq["Exchange"] = nasdaq["Exchange"].fillna("NASDAQ")
